  ...
  methods: {
    ...
    async onSuccess() {
      try {
        ...
        this.$waveui.notify(
          'При редактировании пользователя произошла ошибка: ' + e.message,
          'error',
        )
      } finally {
...
<template>
  ...
  >
    ...
    <template #actions>
      <div class="spacer" />
      <w-button bg-color="error" @click="deleteUser()"> Удалить </w-button>
      <w-button class="mx3" :loading="form.buttonLoading" @click="$refs.editUserForm.validate()">
        Сохранить
      </w-button>
