...
export default defineComponent({
  name: 'UsersView',
  components: { SEditUserDialog, SCreateUserDialog },
  data: () => ({
    table: {
      headers: [
    ...
    filter: {
      ...
      keywordFilter: (filter) => (item) => {
        return (
          new RegExp(filter.keyword[1], 'i').test(item.id) &&
  ...
  methods: {
    ...
    uploadUsersFormOnSuccess() {
      ...
      httpCommon
        .uploadUsers(
          this.uploadUsersDialog.form.fields.file,
          this.uploadUsersDialog.form.fields.rewrite,
        )
        .then(
          () => {
            this.$waveui.notify('Пользователи успешно загружены!', 'success', 0)
...
<template>
  <w-card title="Пользователи" bg-color="base-bg-color" class="ma4">
    <w-flex row class="gap2 mb4">
      <w-button bg-color="success" @click="openCreateUserDialog"> Создать </w-button>
      <w-button bg-color="success" disabled @click="openUploadUsersDialog"> Загрузить </w-button>
      <w-button @click="loadUsers">
  ...
  >
    ...
    <div class="caption">
      Формат файла: CSV (txt) (Разделитель табуляция)<br />Кодировка: UTF-8<br />Порядок данных как
      в таблице (без id)
    </div>
    <w-form
      ref="uploadUsersForm"
    ...
    >
      ...
      <w-checkbox
        ...
        v-model="uploadUsersDialog.form.fields.rewrite"
      >
        Перезаписать <br />
        <div class="caption">Полная очистка базы перед записью</div>
      </w-checkbox>
    </w-form>

    <template #actions>
      <div class="spacer" />
      <w-button class="mr3" @click="$refs.uploadUsersForm.validate()"> Загрузить </w-button>
