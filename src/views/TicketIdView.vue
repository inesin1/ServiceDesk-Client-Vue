<template>
  <w-card v-if="!loading" :title="'Заявка №' + ticket.id" bg-color="base-bg-color" class="ma4">
    <w-flex column gap="4">
      <!--Панель управления-->
      <w-flex gap="4">
        <w-button
        ...
        <div v-if="userStore.user.role.id === 3" class="w-flex row gap2">
          <w-select
            :items="executorSelect.items"
            @item-select="executorSelect.selection = $event.value"
          />

          <w-button bg-color="primary" @click="setExecutor(executorSelect.selection)">
        ...
        <w-button
          v-if="
            ticket.status.id === 3 &&
            ticket.executor.id === userStore.user.id &&
      ...
      <w-flex gap="4">
        <div class="w-flex column xs6 gap4">
          <!--                  <s-select-field
                  ...
                  />-->
          <s-text-field title="Статус" :body="ticket.status.name" />
          <s-text-field title="Источник" :body="ticket.source.name" />
          <s-text-field
            title="Создатель"
          ...
          />
          <s-text-field v-else title="Исполнитель" body="Не назначен" />
        </div>
        <w-divider vertical />
        <div class="w-flex column xs6 gap4">
          <s-text-field title="Дата создания" :body="ticket.createdAt" />
          <s-text-field
            title="Дата закрытия"
            :body="ticket.closedAt !== undefined ? ticket.closedAt : '-'"
          />
          <s-text-field title="Срок" :body="ticket.timeLimit" />
        </div>
      </w-flex>
      <w-divider />

      <div class="w-flex column gap4">
        <s-text-field title="Категория" :body="ticket.category.name" />
        <s-text-field title="Подробности" :body="ticket.details" />
      </div>
      ...
      <div class="w-flex column gap4">
        ...
        <w-flex column gap="4">
          <w-textarea v-model="comments.newComment" outline inner-icon-left="mdi mdi-email" />

          <w-button class="align-self-start" @click="createComment"> Отправить </w-button>
        </w-flex>
      </div>
    </w-flex>
  </w-card>
...
export default {
  ...
  methods: {
      ...
      httpCommon
        ...
        .then((statuses) =>
          statuses.forEach((status) =>
            this.statusSelect.items.push({ label: status.name, value: status.id }),
          ),
        )
    },
    ...
    createComment() {
      httpCommon
        .createTicketComment(this.ticket.id, {
    ...
    closeTicket() {
      httpCommon.closeTicket(this.ticket.id).then(() => {
        this.$waveui.notify('Заявка выполнена!', 'success')
        setTimeout(() => this.loadTicketData(), 1000)
        setTimeout(
          () => this.createSystemComment(`Статус заявки изменен на: ${this.ticket.status}`),
          3000,
        )
      })
    },
